...
main() {
    ...
    if [ -f "$PRIVILEGES_CHECKER_LA" ]; then
        ...
        /usr/bin/logger "privilegeschecker LaunchAgent installation completed"
    ...
    # sleep 10 seconds to let things calmn down
    /bin/sleep 10

    # See if dockutil is installed because we can use it to move the app to the dock. xcfv
