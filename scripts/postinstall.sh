...
main() {
    ...
    if [ -f "$PRIVILEGES_CHECKER_LA" ]; then
        ...
        /bin/launchctl asuser "$current_user_uid" /usr/bin/sudo -u \
            "$current_user" --login /bin/launchctl load "$PRIVILEGES_CHECKER_LA"
    ...
    /bin/launchctl asuser "$current_user_uid" sudo -u "$current_user" --login /Applications/Privileges.app/Contents/Resources/PrivilegesCLI --remove
